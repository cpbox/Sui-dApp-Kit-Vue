...
export const usePersistState = createGlobalState(() =>
  useStorage("sui-vue-connection-info", {
...
export const useWallets = () => {
  const getFilteredWallets = () => {
    const ws = get().filter((wallet): wallet is WalletWithRequiredFeatures =>
      isWalletWithRequiredFeatureSet(wallet, globalState.requiredFeatures.value)
    );
    return [
      ...(globalState.preferredWallets.value
        .map((name) => ws.find((wallet) => wallet.name === name))
        .filter(Boolean) as WalletWithRequiredFeatures[]),
      ...ws.filter((wallet) => !globalState.preferredWallets.value.includes(wallet.name)),
    ];
  };

  const wallets = shallowRef(getFilteredWallets());
  on("register", () => (wallets.value = getFilteredWallets()));
  on("unregister", () => (wallets.value = getFilteredWallets()));
  return { wallets };
};
  ...
  const connect = async (wallet: WalletWithRequiredFeatures) => {
    try {
      ...
      if (persistState.value.lastConnectedAccountAddress) {
        globalState.currentAccount.value =
          connectOutput.accounts.find((a) => a.address === persistState.value.lastConnectedAccountAddress) ??
          connectOutput.accounts[0];
      } else {
...
export const useCurrentWallet = () => ({
  currentWallet: () => globalState.currentWallet,
  currentWalletStatus: globalState.currentWalletStatus,
});
...
export const useSignTransactionBlock = () => {
  ...
  ) => Promise<SignedTransaction | SuiSignTransactionBlockOutput> = ({ transaction, account, chain }) => {
    if (!globalState.currentWallet) {
      throw new Error("No wallet is connected.");
    }
